...
### LinkedList
LinkedList是基于双向链表实现的，插入删除操作性能优异，随机访问能力很差，无容量限制。LinkedList允许null元素。LinkedList还实现了Deque接口，可用作堆栈、队列或双端队列。注意，该类未实现同步，是非线程安全的。
...
#### 实现原理
HashMap底层实现是数组，数组的每一项是一条链（Java8中是链或者红黑树），这样设计的目的是为了综合数组的快速访问性和链表的快速存储性。  
